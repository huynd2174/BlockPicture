...
import { LevelBlockSpawner } from './LevelBlockSpawner';
...
@ccclass('PuzzleDragController')
export class PuzzleDragController extends Component {
    @property(Camera)
    ...
    private resolveHudController(): GameHUDController | null {
        const canvas = find('Canvas') || find('block/Canvas');
        return canvas?.getComponent(GameHUDController) || null;
    }

    private resolveUiCamera(hudController: GameHUDController | null): Camera | null {
    ...
    private getImageLayerWorldCorners(imageLayer: Node): Vec3[] {
        ...
        return localCorners.map((localCorner) => {
            ...
            imageLayer.transformPoint(worldCorner, localCorner);
            return worldCorner;
        });
    }
    ...
    private checkWin(nodesToIgnore: Node[] = []) {
        ...
        for (const child of this.blocksRoot.children) {
            if (nodesToIgnore.includes(child)) continue;
